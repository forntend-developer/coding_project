$(document).ready(function(){
  $(document).ready(function () {
    ...
    $(document).on("click", ".select_box_list li", function (e) {
      ...
      if($(this).parents('.review_sel_swiper')){
        initSelSwiper();
      }
    });
  ...
  function skinToneColor() {
    ...
    function changeBg(scroll) {
      ...
      let colorCgTop2 = $(".skin_tone_linker2").offset().top;
      if (scroll > colorCgTop - 200 && scroll < colorCgTop2 - 200) {
  ...
  //종합결과 페이지
  function myColorResult() {
    const myToneListWrap = $('.my_result .my_tone .tone_list_wrap'),
          toneList = myToneListWrap.find('.tone_list'),
          winW = $(window).width(),
          myLi = $('.my_result_cont .my_tone > li');
    ...
    $(window).on('scroll',function() {
      ...
      if(winW > 768) {
        activePos = $('.my_result_cont').offset().top - 400;
        if(scroll > activePos) {                    
          moveActive();
        }
      } else {
        myLi.each(function() {
          activePos = $(this).offset().top;
          liArr.push(activePos);
        })
        for(let i = 0; i < myLi.length; i++) {
          if(scroll > liArr[i] - 600) {    
            myLi.eq(i).addClass('active');               
            moveActive(i);
          }
        }
      }
    ...
    function moveActive(i) {
      let pos, //pc
          moPosArr = []; //mo
  
      toneList.each(function() {
        let divWid =  $(this).find('div').outerWidth(),
            divWidHalf = divWid/2,
            target = $(this).children('div.active'),
            targetIdx = target.index(),
            boxWid = $(this).width(),
            boxWidHalf = boxWid / 2,
            targetPos; //active 위치
            
            targetPos = divWid * targetIdx + divWidHalf;
  
        if (targetPos == boxWidHalf) {
          pos = 0;
        } else {
          pos = boxWidHalf - targetPos;
        }
        moPosArr.push(pos);
        if(winW > 768) { //pc
      ...
      if(winW <= 768) { //mo
        myLi.eq(i).find(toneList).stop().animate({left:moPosArr[i]});
      } 
    }
  }
  
  //fold Box
  function foldBox(btn) {
    let foldedCont = $(btn).parents('.fold_box').find('.folded_cont');
    if($(btn).hasClass("active")){
      foldedCont.removeClass('show')
      $(btn).removeClass("active")
    }else{
      foldedCont.addClass('show')
      $(btn).addClass("active")
    }
  }
  /* 스크립트 소스 수정한 부분 
